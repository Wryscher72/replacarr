...
import {
  X, Search, Download, ChevronUp, ChevronDown, Loader2,
  AlertCircle, CheckCircle2, Award, Globe, Layers, Filter,
  PackageCheck, Film, RefreshCw, ShieldAlert,
} from "lucide-react";
...
type PackFilter = "all" | "season" | "episode";
...
export function SeasonSearchModal({ seriesId, seriesTitle, seasonNumber, onClose }: Props) {
  const { sonarrApi } = useApi();

  // Fetch state
  const { data: releases, isLoading, error, refetch, isFetching } = useQuery<Release[]>({
    queryKey: ["sonarr-season-releases", seriesId, seasonNumber],
    queryFn: () =>
      sonarrApi
        .get(`/release?seriesId=${seriesId}&seasonNumber=${seasonNumber}`)
        .then((r) => r.data),
    staleTime: 0,
    gcTime: 0,
    retry: false,
  ...
  const grab = useMutation({
    mutationFn: (r: Release) =>
      sonarrApi.post("/release", { guid: r.guid, indexerId: r.indexerId }),
    onSuccess: (_, r) =>
  ...
  const filtered = useMemo(() => {
    return (releases ?? []).filter((r) => {
      if (packFilter === "season" && !r.fullSeason) return false;
      if (packFilter === "episode" && r.fullSeason) return false;
      if (releaseGroupFilter !== "all" && r.releaseGroup !== releaseGroupFilter) return false;
      if (!showRejected && !r.approved) return false;
      return true;
    });
  }, [releases, packFilter, releaseGroupFilter, showRejected]);
  ...
  function SortIcon({ k }: { k: SortKey }) {
    ...
    return sort.dir === "asc"
      ? <ChevronUp size={10} className="text-[var(--color-sonarr)]" />
      : <ChevronDown size={10} className="text-[var(--color-sonarr)]" />;
  }

  const totalCount = releases?.length ?? 0;
  const seasonPackCount = releases?.filter((r) => r.fullSeason).length ?? 0;
        ...
        >
          ...
          <div className="flex items-center gap-3 px-5 py-4 border-b border-[var(--color-border)] shrink-0">
            <div className="w-9 h-9 rounded-lg flex items-center justify-center"
              style={{ background: "color-mix(in srgb, var(--color-sonarr) 15%, transparent)", border: "1px solid color-mix(in srgb, var(--color-sonarr) 35%, transparent)" }}>
              <Search size={15} className="text-[var(--color-sonarr)]" />
            </div>
            <div className="min-w-0">
              <h2 className="text-sm font-bold text-white">
                Season {seasonNumber} — Release Search
              </h2>
              <p className="text-xs text-slate-300 truncate">{seriesTitle}</p>
            </div>
          ...
          {!isLoading && !error && totalCount > 0 && (
            <div className="flex flex-wrap items-center gap-2 px-5 py-2.5 border-b border-[var(--color-border)] bg-[var(--color-bg-base)]/40 shrink-0">
              {/* Pack type filter */}
              <div className="flex items-center gap-1 bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-lg p-0.5">
                {([ 
                  { value: "all",     label: "All",          icon: <Layers size={10} /> },
                  { value: "season",  label: `Season Packs (${seasonPackCount})`, icon: <PackageCheck size={10} /> },
                  { value: "episode", label: "Singles",      icon: <Film size={10} /> },
              ...
              {releaseGroups.length > 0 && (
                <div className="flex items-center gap-1.5">
                  ...
                  <select
                    ...
                    className="px-2 py-1 rounded-lg bg-[var(--color-bg-card)] border border-[var(--color-border)] text-white text-[11px] outline-none focus:border-[var(--color-sonarr)] transition-colors"
                  >
              ...
              <div className="flex items-center gap-0.5 bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-lg px-1 py-0.5">
                {([0.8, 0.9, 1.0, 1.15] as const).map((z, i) => (
                  <button
                    ...
                    style={{ fontSize: `${10 + i * 2}px`, background: rowZoom === z ? "color-mix(in srgb, var(--color-sonarr) 30%, transparent)" : undefined }}
                  >
                    A
                  </button>
                ))}
            ...
            {!isLoading && !error && sorted.length > 0 && (
              <table className="w-full text-xs" style={{ zoom: rowZoom }}>
                <thead className="sticky top-0 bg-[var(--color-bg-card)] border-b border-[var(--color-border)] z-10">
                  <tr>
                    <th className="px-4 py-2.5 text-left w-0">
                      <span className="text-slate-200 font-semibold uppercase tracking-wider">Type</span>
                    </th>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-2.5">
                          {r.fullSeason ? (
                            <span className="flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold whitespace-nowrap bg-[var(--color-sonarr)]/15 text-[var(--color-sonarr)] border border-[var(--color-sonarr)]/25">
                              <PackageCheck size={9} /> Season
                            </span>
                          ) : (
                            <span className="flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold whitespace-nowrap bg-slate-500/15 text-slate-400 border border-slate-500/20">
                              <Film size={9} /> Single
                            </span>
                          )}
                        </td>
                        ...
                        <td className="px-4 py-2.5 text-right">
                          {isGrabbed ? (
                            ...
                            <button
                              ...
                              className="flex items-center justify-end gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-semibold transition-all ml-auto disabled:opacity-50"
                              style={{
                                background: "color-mix(in srgb, var(--color-sonarr) 15%, transparent)",
                                color: "var(--color-sonarr)",
                                border: "1px solid color-mix(in srgb, var(--color-sonarr) 35%, transparent)",
                              }}
                            >
                        ...
                        <td className="px-3 py-2.5 text-right whitespace-nowrap">
                          {r.customFormatScore != null ? (
                            <div className="relative group/cf inline-flex items-center justify-end">
                              <span className="flex items-center gap-0.5 text-[11px] font-bold font-mono cursor-help"
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                releaseGroupFilter === r.releaseGroup
                                  ? "text-[var(--color-sonarr)] border-[var(--color-sonarr)]/30 bg-[var(--color-sonarr)]/10"
                                  : "text-slate-200 border-slate-500/40 bg-slate-700/20 hover:border-[var(--color-sonarr)]/30 hover:text-[var(--color-sonarr)]"
                              )}
          ...
          {!isLoading && !error && totalCount > 0 && (
            <div className="px-5 py-3 border-t border-[var(--color-border)] shrink-0 flex flex-wrap items-center gap-3 text-[11px] text-slate-400">
              ...
              <span>·</span>
              <span><span className="text-[var(--color-sonarr)] font-semibold">{seasonPackCount}</span> season packs</span>
              <span>·</span>
