  ...
  return (
    <div className="border-b border-[var(--color-border)]/40 last:border-0">
      ...
      >
        ...
        {/* Title + all metadata badges stacked */}
...
export default function SonarrDetailPage() {
  ...
  // Rich episode file data — includes mediaInfo, releaseGroup, customFormats, customFormatScore, languages
  ...
  const applyMonitorMutation = useMutation({
    mutationFn: async ({ strategy, monitorNewItems }: { strategy: MonitorStrategy; monitorNewItems: "all" | "none" }) => {
      ...
      // 1. Update series with seasons + monitorNewItems
      ...
      // 2. Cascade episode monitor state
  ...
  // Build lookup map: episodeFileId → EpisodeFile
    ...
    return (
      <div className="flex flex-col min-h-screen">
        ...
        <div className="flex flex-col items-center justify-center flex-1 gap-3">
          ...
          <Link href="/sonarr" className="text-[var(--color-sonarr)] text-sm hover:underline">
            ← Back to library
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <Header title={series.title} subtitle={`${series.year} · ${series.network}`} />
      ...
      <div className="flex-1">
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-[300px_1fr]">

          {/* ─── LEFT SIDEBAR ─── */}
          <aside className="lg:border-r border-[var(--color-border)] p-5 space-y-6">
            {/* Poster */}
            <div className="flex gap-4 lg:block lg:space-y-0">
              <div className="relative aspect-[2/3] w-24 lg:w-full lg:max-w-[160px] rounded-xl overflow-hidden border border-[var(--color-border)] shadow-xl shrink-0 mx-auto hidden lg:block">
                {poster ? (
                  <Image src={poster} alt={series.title} fill className="object-cover" sizes="(max-width: 1024px) 96px, 160px" />
                ) : (
                  <div className="w-full aspect-[2/3] bg-[var(--color-bg-card)] flex items-center justify-center">
                    <Tv2 size={24} className="text-[var(--color-text-muted)]" />
                  </div>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-4">
              ...
              {/* Root Folder */}
              <div className="col-span-2">
                <p className="text-[10px] font-semibold text-[var(--color-text-muted)] uppercase tracking-wider mb-0.5">Root Folder</p>
                {editingRootFolder ? (
                  <div className="flex items-center gap-1.5">
                    <select
                  ...
                  <button onClick={() => { setEditingRootFolder(true); setSelectedRootFolderPath(series.path); }} className="flex items-center gap-1 text-xs text-white hover:text-[var(--color-sonarr)] transition-colors group font-mono truncate max-w-full" title={series.path}>
                    <FolderOpen size={11} className="shrink-0 opacity-50 group-hover:opacity-100" />
              ...
              <div className="col-span-2">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-[10px] font-semibold text-[var(--color-text-muted)] uppercase tracking-wider">Tags</p>
            ...
            <div className="flex flex-col gap-2">
              <Link href="/sonarr" className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-[var(--color-text-secondary)] hover:text-white border border-[var(--color-border)] hover:border-[var(--color-border-bright)] transition-all">
                  ...
                  {showMonitorDialog && (
                    ...
                    >
                      <div className="mt-1 p-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] space-y-3">
                        ...
                        {/* Monitor new items toggle (only shown when not "none") */}
